...
def top5_countries_by_date(day: int, month: int, year: int = 2020) -> List[str]:
    ...
    d = datetime.date(year,month,day)
    dl = d.strftime('%m/%d/%y').lstrip("0").replace(" 0", " ").replace("/0", "/")
    countries = confirmed_cases[["Country/Region", dl]].groupby(["Country/Region"]).sum().sort_values(by=dl, ascending=False).head(5)
...
def no_new_cases_count(day: int, month: int, year: int = 2020) -> int:
    ...
    return len(confirmed_cases.loc[confirmed_cases[dl]-confirmed_cases[wczorajstr]!=0].index)
